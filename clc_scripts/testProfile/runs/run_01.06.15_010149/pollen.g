...
@after{
   ...
   varDeref[boolean postExpr]
    :	(qualName '(') => qualName fcnArgumentList  (arrayAccess)? ( '.' varDeref[true])?
    	-> ^(E_CALL<ExprNode.Call>["E_CALL"] ^(E_IDENT<ExprNode.Ident>["E_IDENT", $postExpr] qualName) fcnArgumentList (arrayAccess)? varDeref?)
     |    (qualName '[') => qualName arrayAccess (fcnArgumentList)? (arrayAccess)? ('.' varDeref[true])?
...
@init {
    ...
    :     (builtinType) => stmtLeadingVar
    |    ( INJECT assign) => stmtAssignInject
    |     stmtInjection
    |    (exprUnary) => stmtLeadingVar
    |    stmtBlock
    |    stmtPrint
    |    stmtReturn
    |    stmtBreak
...
//    |    (qualName IDENT ASSIGN) => varDeclList                              ambig!
//    |    (typeName IDENT '[') => varArray                                             ambig!
