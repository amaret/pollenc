...
exprUnary
 ...
 //   |    exprNew can get this via varOrFcnorArray
    ...
    |    exprPrimary
    //exprPost[$exprPrimary.tree]    -> exprPost    
    ;
exprPrimary
    :    primitiveLit
    ...
    ;
    /*
exprPost[CommonTree expr]
     :     incDecOp  -> ^(E_UNARY<ExprNode.Unary>["E_UNARY", true] {expr} incDecOp)
     |
     ;  
     */
    /*
    ...
    |    incDecOp exprPrimary         -> ^(E_UNARY<ExprNode.Unary>["E_UNARY"] exprPrimary incDecOp)
//    |    postExprPrimary
   |    exprPrimary incDecOp         -> ^(E_UNARY<ExprNode.Unary>["E_UNARY", true] exprPrimary incDecOp)
    ;
exprPrimary     
     :	 primitiveLit
    |    '(' expr ')'                -> ^(E_PAREN<ExprNode.Paren>["E_PAREN"]  expr)
...
//     |     varDeref[false] 
    ...
    ;
 /*
varDeref[boolean postExpr]
    :	(qualName '(') => varDerefCall[postExpr] // ('.'! varDeref[true])?
//    	-> ^(E_CALL<ExprNode.Call>["E_CALL"] ^(E_IDENT<ExprNode.Ident>["E_IDENT", $postExpr] qualName) fcnArgumentList)
 //   	-> ^(E_CALL<ExprNode.Call>["E_CALL"] ^(E_IDENT<ExprNode.Ident>["E_IDENT", $postExpr] qualName) fcnArgumentList (arrayAccess)? varDeref?)
     |    (qualName '[') => varDerefArr[postExpr]   //('.'! varDeref[true])?
     	//-> ^(E_IDENT<ExprNode.Ident>["E_IDENT", $postExpr] qualName arrayAccess fcnArgumentList? ) //(arrayAccess)? varDeref?)
//     |     qualName  
//            -> ^(E_IDENT<ExprNode.Ident>["E_IDENT", $postExpr] qualName)
    ;
varDerefCall[boolean postExpr] 
    :	qualName fcnArgumentList //(arrayAccess)? //('.' varDeref[true])? 
       	-> ^(E_CALL<ExprNode.Call>["E_CALL"] ^(E_IDENT<ExprNode.Ident>["E_IDENT", $postExpr] qualName) fcnArgumentList) // arrayAccess? varDeref?)
    ;
varDerefArr[boolean postExpr]
    :	qualName arrayAccess (fcnArgumentList)? //('.' varDeref[true])?
         	-> ^(E_IDENT<ExprNode.Ident>["E_IDENT", $postExpr] qualName arrayAccess fcnArgumentList?) 
    ;
    */
...
stmtBlock
    :    /*(NLL)?*/ braceOpenNLL stmts braceCloseNLL     -> ^(S_BLOCK<StmtNode.Block>["S_BLOCK"] stmts)
    ;
stmts
...
//    :     (qualName fcnRefTypeList IDENT (ASSIGN qualName)? delim) => varFcnRef
//    |     ('(' rtnType typeName fcnRefTypeList ')' IDENT (ASSIGN expr)? delim) => varFcnRef2  
...
@init {
 ...
 //   :     (typeName IDENT (ASSIGN expr)? delim) => varDeclList
